 ...
 */
static int32_t AnglePID(float set, float nextPoint)//, int32_t VC_PWM) 
{ 
    float error;
...
//    int32_t changeRate;
//    int32_t error;
//    static int32_t preError = 0;
//	static int16_t cnt = 0;
//    AcceGet(&mmax, &mmaz);

//    angle = atan2f((float)mmax,(float)mmaz) * 180 / PI;
...
//	if(cnt > 200)
//	{
//		Pre_Angle_Kalman = (int16_t)Angle_Kalman;
//		cnt = 0;
//	}
//        error = set - Angle_Kalman;
//        changeRate = (error - preError);
//        preError = error;


    return -AnglePID(MODE.AC_Set, Angle_Kalman);//, VC_Out);//暂且取相反数+ MODE.VC_Set * 0.3- change * 0.10
}
